...
export function parseArchiveReference(sourceReference, defaultKey) {
  ...
  if (sourcePath !== APPROVED_ARCHIVE_REFERENCE && sourcePath !== APPROVED_ARCHIVE_PATH) {
    throw new Error('Archive source must be ~/W3/Rets/archive-rpc.env');
  }
  if (!ALLOWED_KEYS.has(key)) throw new Error('Archive reference key is not approved');
  return { filePath: APPROVED_ARCHIVE_PATH, key, display: `${APPROVED_ARCHIVE_REFERENCE}:${key}` };
}
