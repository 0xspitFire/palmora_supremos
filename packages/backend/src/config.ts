...
export interface SecretReference { store: SecretStore; key: 'ETHEREUM_RPC_URL' | 'ROBINHOOD_RPC_URL' | 'INK_RPC_URL' | 'BASE_RPC_URL' | 'OPENSEA_API' | 'ALCHEMY_API' | 'TEST_WALLET_PK' | 'TEST_WALLET_ADDR' | 'TG_BOT_TOKEN' | 'TG_CHAT_ID'; }
...
export function assertSafeConfig(config: BackendConfig): void {
  ...
  if (config.chainId === 4663 && config.archiveReference !== `${MAINNET_SECRET_FILE_REFERENCE}:ROBINHOOD_RPC_URL`) throw new Error('ROBINHOOD_ARCHIVE_REFERENCE_INVALID');
}
