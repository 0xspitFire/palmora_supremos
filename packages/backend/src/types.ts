...
export interface NotificationOutboxRecord { id: string; sourceEventId: string; runId?: string; type: string; text: string; state: 'pending' | 'delivering' | 'delivered'; attempts: number; createdAt: string; deliveredAt?: string; retentionUntil?: string; nextAttemptAt?: string; deliveryLeaseUntil?: string; lastError?: string; }
...
export interface ReceiptRecord { id: string; executionId: string; runId: string; transactionAttemptId?: string; state: ExecutionState; robinhoodFinality?: RobinhoodFinality; blockNumber?: bigint; blockHash?: string; actualSpendWei?: bigint; observedAt: string; }
...
export interface OperationalReadiness { secretStoreReference: string; storePath: string; signerReady: boolean; killSwitchEngaged: boolean; notificationReady: boolean; chainVerification: ChainVerificationStatus; lastReconciliationAt: string; observedAt: string; expiresAt: string; }
export interface RuntimeStatus { startupState: StartupState; reconciliationCompletedAt?: string; blockingReasons: string[]; dependencies: DependencyReadiness; operational?: OperationalReadiness; }
