...
describe('database migrations and spend reservations', () => {
  ...
  it('serializes concurrent migration startup without stale migration reads', async () => {
    ...
    expect(results.sort()).toEqual(['20', '20']);
    rmSync(directory, { recursive: true, force: true });
  });
  ...
  it('backs up and restores the durable schema without secrets', async () => {
    ...
    expect(verification.schemaVersion).toBe(20);
    ...
    expect(restoredVerification).toMatchObject({ schemaVersion: 20, integrityCheck: 'ok', foreignKeyViolations: 0 });
  ...
  it('records retention evidence alongside backup verification evidence', () => {
    ...
    repository.recordBackupRestoreEvidence({ id: 'backup-evidence-integrity', storeReference: 's3://test/store', backupReference: 's3://test/backup', sha256: 'a'.repeat(64), schemaVersion: 20, operation: 'verification', outcome: 'passed', killSwitchEngaged: true, encryptionVerified: true, integrityCheck: 'ok', verificationSha256: 'a'.repeat(64), evidence: { offHost: true, restoreVerified: true, postRestoreReconciliation: true, retentionDays: 30 }, recordedAt: '2026-01-01T00:00:02.000Z' });
  ...
  it('maps opportunity evidence and rejects fingerprint or backup evidence forgery', () => {
    ...
    expect(() => repository.recordBackupRestoreEvidence({ id: 'forged-backup', storeReference: 'store', backupReference: 'backup', sha256: 'a'.repeat(64), schemaVersion: 20, operation: 'verification', outcome: 'passed', killSwitchEngaged: false, recordedAt: '2026-01-01T00:00:00.000Z' })).toThrow('passed backup evidence requires encrypted');
    db.close();
  });
