...
describe('database migrations and spend reservations', () => {
  ...
  it('records retention evidence alongside backup verification evidence', () => {
    ...
    repository.recordBackupRestoreEvidence({ id: 'backup-evidence-integrity', storeReference: 'temporary-store', backupReference: 'temporary-backup', sha256: 'a'.repeat(64), schemaVersion: 16, operation: 'verification', outcome: 'passed', killSwitchEngaged: true, encryptionVerified: true, integrityCheck: 'ok', recordedAt: '2026-01-01T00:00:02.000Z' });
  ...
  it('maps opportunity evidence and rejects fingerprint or backup evidence forgery', () => {
    ...
    expect(() => repository.recordBackupRestoreEvidence({ id: 'forged-backup', storeReference: 'store', backupReference: 'backup', sha256: 'a'.repeat(64), schemaVersion: 16, operation: 'verification', outcome: 'passed', killSwitchEngaged: false, recordedAt: '2026-01-01T00:00:00.000Z' })).toThrow('passed backup evidence requires encryption');
    db.close();
  });
